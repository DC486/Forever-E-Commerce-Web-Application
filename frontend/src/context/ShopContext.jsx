...
const ShopContextProvider = (props) => {
  ...
  const toggleWishlist = async (productId) => {
    ...
    if (!wishlist.some((item) => item._id === productId)) {
      ...
      try {
        const response = await axios.post(`${backendUrl}/api/user/remove`, {
          id: productId,
          email: localStorage.getItem("email"),
        });
  ...
  const whishlistData = async () => {
    const res = await axios.post(backendUrl + "/api/user/likeItem", {
      email: localStorage.getItem("email"),
    });
  ...
  const checkSubscription = async () => {
    try {
      ...
      const response = await axios.post(backendUrl + "/api/user/IsSubscribe", {
        email,
      });
      console.log(email);
      ...
      if (response.data) setSubscribed(response.data.status);
    } catch (error) {}
  };
